...
class ZLHomeMiddleCell: ZLHomeTopicCellTableViewCell {
    ...
    var newsTopic:ZLNewsTopic?{
        didSet{
            ...
            if let commentCount = newsTopic?.comment_count{
                commentCount >= 10000 ? (commentLabel.text = "\(commentCount / 10000)万评论") : (commentLabel.text = "\(commentCount)条评论")
            }
            
            if (newsTopic!.titleH + avatarImageView.height + kMargin) < newsTopic?.imageH{
                closeButton.snp_remakeConstraints(closure: { (make) in
                    make.right.equalTo(rightImageView.snp_left).offset(-kMargin)
